...
function getDashboardData() {
  ...
  // STREAM B: Get ALL data from Raw_Data (for PENDING ENTRIES ONLY - not for KPI/charts)
  ...
  // Get PENDING entries only from Raw_Data
  var pendingData = rawData.filter(function(r) {
    return r.status === 'PENDING_REVIEW';
  }).slice(-50).reverse();
  
  // Get last 50 entries from APPROVED data (Final_Data) - NOT from Raw_Data
